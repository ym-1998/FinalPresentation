...
disp("创建primal-dual条件引入新变量...");
Lanta_meo=sdpvar(Time,1);Gama_meo=sdpvar(Time,1);
XE=sdpvar(Time,1);
YE=sdpvar(Time,1);
XG=sdpvar(Time,1);
YG=sdpvar(Time,1);
...
%各项收益建模
    ...
    Cons_MPEC=[Cons_MPEC,Lanta_meo==Lanta'*Ameo];Cons_MPEC=[Cons_MPEC,Gama_meo==Gama'*Anm];
    
    Cons_MPEC=[Cons_MPEC,Fai==sum(YE)];%购电成本，$   
    Cons_MPEC=[Cons_MPEC,Faigas==sum(YG)];%购气成本，$ 
    Cons_MPEC=[Cons_MPEC,YE>=Lanta_meo.*Pmeomin+Cmeomin.*Pmeo-Cmeomin.*Pmeomin];%McCormickEnvelope
    Cons_MPEC=[Cons_MPEC,YE<=Lanta_meo.*Pmeomin+Cmeomax.*Pmeo-Cmeomax.*Pmeomin];
    Cons_MPEC=[Cons_MPEC,YE<=Lanta_meo.*Pmeomax+Cmeomin.*Pmeo-Cmeomin.*Pmeomax];
    Cons_MPEC=[Cons_MPEC,YE>=Lanta_meo.*Pmeomax+Cmeomax.*Pmeo-Cmeomax.*Pmeomax];
    Cons_MPEC=[Cons_MPEC,YG>=Gama_meo.*Gmeomin+Cmeogasmin.*Gmeo-Cmeogasmin.*Gmeomin];%McCormickEnvelope
    Cons_MPEC=[Cons_MPEC,YG<=Gama_meo.*Gmeomin+Cmeogasmax.*Gmeo-Cmeogasmax.*Gmeomin];
    Cons_MPEC=[Cons_MPEC,YG<=Gama_meo.*Gmeomax+Cmeogasmin.*Gmeo-Cmeogasmin.*Gmeomax];
    Cons_MPEC=[Cons_MPEC,YG>=Gama_meo.*Gmeomax+Cmeogasmax.*Gmeo-Cmeogasmax.*Gmeomax];
...
disp("创建下层电力市场问题强对偶约束条件...");
% Cons_MPEC=[Cons_MPEC,sum(sum(CG_temp.*PG))-sum(Cmeo.*Pmeo)==sum(sum(Lanta.*PD))-pi*(sum(sum(EpsilonMax+EpsilonMin)))-sum(sum(MiuMax.*PGmax_temp))-sum(sum(Vmin.*PLmax_temp))-sum(sum(Vmax.*PLmax_temp))];
Cons_MPEC=[Cons_MPEC,sum(sum(CG_temp.*PG))-sum(XE)==sum(sum(Lanta.*PD))-pi*(sum(sum(EpsilonMax+EpsilonMin)))-sum(sum(MiuMax.*PGmax_temp))-sum(sum(Vmin.*PLmax_temp))-sum(sum(Vmax.*PLmax_temp))];
Cons_MPEC=[Cons_MPEC,XE>=Cmeo.*Pmeomin+Cmeomin.*Pmeo-Cmeomin.*Pmeomin];
Cons_MPEC=[Cons_MPEC,XE<=Cmeo.*Pmeomin+Cmeomax.*Pmeo-Cmeomax.*Pmeomin];
Cons_MPEC=[Cons_MPEC,XE<=Cmeo.*Pmeomax+Cmeomin.*Pmeo-Cmeomin.*Pmeomax];
Cons_MPEC=[Cons_MPEC,XE>=Cmeo.*Pmeomax+Cmeomax.*Pmeo-Cmeomax.*Pmeomax];
...
% disp("创建下层天然气市场强对偶条件约束...");
% Cons_MPEC=[Cons_MPEC,sum(sum(repmat(Cwgas,1,Time).*Qw))-Cmeogas'*Gmeo==-sum(sum(Beta1Max.*repmat(Qwmax,1,Time)))-sum(sum((Beta2Min+Beta2Max).*repmat(Qlgmax,1,Time)))-sum(sum(Beta3Max.*repmat(Qcmax,1,Time)))-Gmax*sum(Beta4Max)+sum(sum(Gama.*(Andg*Qdg)))];
Cons_MPEC=[Cons_MPEC,sum(sum(repmat(Cwgas,1,Time).*Qw))-sum(XG)==-sum(sum(Beta1Max.*repmat(Qwmax,1,Time)))-sum(sum((Beta2Min+Beta2Max).*repmat(Qlgmax,1,Time)))-sum(sum(Beta3Max.*repmat(Qcmax,1,Time)))-Gmax*sum(Beta4Max)+sum(sum(Gama.*(Andg*Qdg)))];
Cons_MPEC=[Cons_MPEC,XG>=Cmeogas.*Gmeomin+Cmeogasmin.*Gmeo-Cmeogasmin.*Gmeomin];
Cons_MPEC=[Cons_MPEC,XG<=Cmeogas.*Gmeomin+Cmeogasmax.*Gmeo-Cmeogasmax.*Gmeomin];
Cons_MPEC=[Cons_MPEC,XG<=Cmeogas.*Gmeomax+Cmeogasmin.*Gmeo-Cmeogasmin.*Gmeomax];
Cons_MPEC=[Cons_MPEC,XG>=Cmeogas.*Gmeomax+Cmeogasmax.*Gmeo-Cmeogasmax.*Gmeomax];
