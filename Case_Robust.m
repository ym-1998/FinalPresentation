%% 鲁棒优化案例
clc;
%% 鲁棒优化模型+有CHP机组+电热锅炉+储能技术---下层包括电力市场和天然气市场
Input;
tic;%将建模的时间包括在内
%% 决策变量
disp("创建上层问题决策变量...");
...
%各项收益建模
    ...
    Cons_MPEC=[Cons_MPEC,Obj_MPEC<=Z(s)];%%最终的期望收益，$
    end
%% 创建约束条件
disp('创建上层问题约束条件...');
%CHP机组建模
...
%% 获取计算结果
Res_Case4_MPEC.Pchp=double(Pchp);
Res_Case4_MPEC.Hchp=double(Hchp);
Res_Case4_MPEC.Ichp=double(Ichp);
Res_Case4_MPEC.SUchp=double(SUchp);
Res_Case4_MPEC.SDchp=double(SDchp);
Res_Case4_MPEC.GCchp=double(GCchp);
Res_Case4_MPEC.Peb=double(Peb);
Res_Case4_MPEC.Heb=double(Heb);
Res_Case4_MPEC.Pwt=double(Pwt);
Res_Case4_MPEC.Pch=double(Pch);
Res_Case4_MPEC.Pdis=double(Pdis);
Res_Case4_MPEC.Hch=double(Hch);
Res_Case4_MPEC.Hdis=double(Hdis);
Res_Case4_MPEC.Gch=double(Gch);
Res_Case4_MPEC.Gdis=double(Gdis);
Res_Case4_MPEC.Ap=double(Ap);
Res_Case4_MPEC.Ah=double(Ah);
Res_Case4_MPEC.Ag=double(Ag);
Res_Case4_MPEC.Pmeo=double(Pmeo);
% Res_Case4_MPEC.Ggm=double(Ggm);
Res_Case4_MPEC.T1=double(T1);
Res_Case4_MPEC.Z=double(Z);
Res_Case4_MPEC.Obj_MPEC=double(Obj_MPEC);
Res_Case4_MPEC.PG=value(PG);
Res_Case4_MPEC.Delta=value(Delta);
Res_Case4_MPEC.Pmeo=value(Pmeo);
Res_Case4_MPEC.Lanta=value(Lanta);
Res_Case4_MPEC.MiuMax=value(MiuMax);
Res_Case4_MPEC.MiuMin=value(MiuMin);
Res_Case4_MPEC.MiuMeoMax=value(MiuMeoMax);
Res_Case4_MPEC.MiuMeoMin=value(MiuMeoMin);
Res_Case4_MPEC.Vmax=value(Vmax);
Res_Case4_MPEC.Vmin=value(Vmin);
Res_Case4_MPEC.EpsilonMax=value(EpsilonMax);
Res_Case4_MPEC.EpsilonMin=value(EpsilonMin);
Res_Case4_MPEC.Epsilon=value(Epsilon);
Res_Case4_MPEC.Cmeo=value(Cmeo);
Res_Case4_MPEC.U_MiuMax=value(U_MiuMax);
Res_Case4_MPEC.U_MiuMin=value(U_MiuMin);
Res_Case4_MPEC.U_MiuMeoMax=value(U_MiuMeoMax);
Res_Case4_MPEC.U_MiuMeoMin=value(U_MiuMeoMin);
Res_Case4_MPEC.U_Vmax=value(U_Vmax);
Res_Case4_MPEC.U_Vmin=value(U_Vmin);
Res_Case4_MPEC.U_EpsilonMax=value(U_EpsilonMax);
Res_Case4_MPEC.U_EpsilonMin=value(U_EpsilonMin);
Res_Case4_MPEC.Fai=value(Fai);
Res_Case4_MPEC.MCP=value(Lanta(5,:))';
Res_Case4_MPEC.Fai=value(Fai);
Res_Case4_MPEC.Faigas=value(Faigas);
Res_Case4_MPEC.Z=value(Z);
Res_Case4_MPEC.ExpectedProfit=dot(Probability,value(Z));%期望利润
Res_Case4_MPEC.WorstProfit=min(value(Z));%最差利润



Res_Case4_MPEC.Qw=value(Qw);
Res_Case4_MPEC.Qlg=value(Qlg);
Res_Case4_MPEC.Qc=value(Qc);
Res_Case4_MPEC.Gmeo=value(Gmeo);
Res_Case4_MPEC.Gama=value(Gama);%dual求出对偶变量值有时需要取负号需要自己判断下
Res_Case4_MPEC.Cmeogas=value(Cmeogas);
Res_Case4_MPEC.Beta1Min=value(Beta1Min);Res_Case4_MPEC.Beta1Max=value(Beta1Max);
Res_Case4_MPEC.Beta2Min=value(Beta2Min);Res_Case4_MPEC.Beta2Max=value(Beta2Max);
Res_Case4_MPEC.Beta3Min=value(Beta3Min);Res_Case4_MPEC.Beta3Max=value(Beta3Max);
Res_Case4_MPEC.Beta4Min=value(Beta4Min);Res_Case4_MPEC.Beta4Max=value(Beta4Max);
Res_Case4_MPEC.U_Beta3Min=value(U_Beta3Min);Res_Case4_MPEC.U_Beta3Max=value(U_Beta3Max);

Res_Case4_MPEC.RR=zeros(numCluster,1);
for s=1:numCluster
    Res_Case4_MPEC.RR(s)=1-Res_Case4_MPEC.Z(s)/Z_optimal(s);
end
Res_Case4_MPEC.MRR=max(Res_Case4_MPEC.RR);